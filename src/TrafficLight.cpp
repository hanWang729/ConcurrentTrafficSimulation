...
void TrafficLight::waitForGreen()
{
    ...
    while(true)
    {
        ...
        if(tlp == TrafficLightPhase::green) {break;}
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::srand(time(0));
    double timeDuration = (double) (4000 + rand() % 2000) / 1000;
    time_t t1 = time(0);
    while(true){
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        time_t t2 = time(0);
        if (difftime(t2,t1) > timeDuration){
            ...
            t1 = time(0);
            timeDuration = (double) (4000 + rand() % 2000) / 1000;
        }
